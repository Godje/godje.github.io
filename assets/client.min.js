...
module.exports = function (c) {
	var $ = c.getContext("2d"),
	    ...
	    randomFromArray = function randomFromArray(arr) {
		var l = arr.length;
	...
	var Runner = function () {
		...
		_createClass(Runner, [{
			...
			value: function update() {
				...
				Math.random() < opts.deathChance ? this.die() : this.move();
			}
		}, {
			key: "move",
			value: function move() {
	...
	function setup() {
		for (var i = 0; i < opts.startRunners; i++) {
...
},{"./background.js":1,"./pages/Home.js":3,"./pages/Portfolio.js":4,"mithril":6,"mithril-stream":5}],3:[function(require,module,exports){
"use strict";
...
var Home = {
	...
	view: function view(vnode) {
		return m("div.home", [m("div.row.first-row", [m("div.col", [m("img", {
			src: "./assets/images/avatar.jpg",
			width: 200,
			height: 200

		})]), m("div.col", [m("h1.name", [m("span", "d"), "aniel ", m("span", "m"), "ayovskiy"]), m("p.job", "Frontend Web Developer"), m("a[href='/portfolio'].mywork-link", { oncreate: m.route.link }, "see my work")])]), m("div.row.row--no-gutter.second-row", [m("div.col.col--4-of-8", [m("h2", "Me and my code at:"), m("div.list", [this.data.links.map(function (link) {
			return m("a[href=" + link.href + "]", {
				target: "_blank"
			}, link.text);
		})])]), m("div.col.col--4-of-8", [m("h2", "Contact me:"), m("div.list", [m("a[href='mailto:" + this.data.email + "']", this.data.email)])])])]);
	}
};

module.exports = Home;

},{"mithril":6,"mithril-stream":5}],4:[function(require,module,exports){
"use strict";
...
var Portfolio = {
	oninit: function oninit(vnode) {
		...
		this.ctrl = {
			...
			slideRight: function slideRight() {
				that.model.entryOverflow() < 0 ? that.model.carouselSlid(that.model.carouselSlid() - 280) : void 0;
				setTimeout(that.ctrl.getOverflow, that.model.slideTransition());
			},
			getOverflow: function getOverflow() {
				var rightPos = that.model.lastentryDOM().getBoundingClientRect().right;
				var containerRight = that.model.containerDOM().getBoundingClientRect().right;
	...
	view: function view(vnode) {
		var that = this;
		return m("div.portfolio", [m("div.container", {
			oncreate: function oncreate(vnode) {
				return that.model.containerDOM(vnode.dom);
			}
		}, [m("div.title", [m("h1", "My Work"), m("a.link-back", {
			href: "/",
			oncreate: m.route.link
		}, "")]), m("div.section", [m("div.section-title", [m("h2", "Tutorials")]), m("div.section-content.tutorials", [this.model.carouselSlid() < 0 ? m("div.arrow.left", {
			onclick: that.ctrl.slideLeft
		}, ["<"]) : "", this.model.jsonLoaded() ? m("div.carousel", {
			onupdate: function onupdate(vnode) {
			...
			return m("a.entry.large[href=" + t.href + "]", {
				...
				oncreate: function oncreate(vnode) {
					arr.length - 1 == ind ? (that.model.lastentryDOM(vnode.dom), that.ctrl.getOverflow(), vnode.dom.addEventListener("onanimationend", that.ctrl.getOveflow)) : void 0;
				},
				style: "background-image: url(" + t.image + ");"
			}), m("div.entry-info", [m("p.title", t.title), m("p.stack", t.type)])]);
		})]) : "Loading...", m("div", {
			className: "arrow right" + (that.model.entryOverflow() < 0 ? "" : " disabled"),
			onclick: this.ctrl.slideRight
		}, [">"])])]), m("div.section", [m("div.section-title", [m("h2", "Canvas Animations")]), m("div.section-content.canvas-animations", [!this.model.jsonLoaded() ? "Loading..." : this.data()["canvas-animations"].map(function (ca) {
			return m("a.entry", {
				href: ca[1],
				target: "_blank"
			}, [m("span", ca[0]), m("p", "(image is loading or missing)"), m("div.entry-image", {
...
},{"mithril":6,"mithril-stream":5}],5:[function(require,module,exports){
/* eslint-disable */
;(function() {
"use strict"
/* eslint-enable */
